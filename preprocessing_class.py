...
class PreProcess:
    ...
    """
    Looks for most appropriate template from template list
    and returns transformed image (perspective transformation)
    if such template was found, then creates ImgProc object to 
    process regions of inerest (ROIs)
    """
    ...
    def __vote(self):
        ...
        for count, template in enumerate(self.template_list):
            matches = self.bf_matcher.knnMatch(template.descriptors,
                                             self.explored_page.descriptors,
                                             k=2)
            for m, n in matches:
    ...
    def __transform(self, index):
        matches = self.bf_matcher.knnMatch(self.template_list[index].descriptors,
                                           self.explored_page.descriptors, k=2)
        keypoints1 = self.template_list[index].keypoints
        keypoints2 = self.explored_page.keypoints
        ...
        # Deskew image
        img_out = cv2.warpPerspective(self.explored_page.image, np.linalg.inv(M),
                                      (self.template_list[index].image.shape[1], 
                                       self.template_list[index].image.shape[0]
                                      )
                                     )
        return img_out

    # This method combines all above together
    def process(self):
